...
use Drupal\drupalmoduleupgrader\Utility\ParametricRewriter\FormStateRewriter;
...
class FormConverter {
  ...
  public function __construct(TargetInterface $target, $form_id, RewriterInterface $rewriter) {
    ...
    $this->builder = $indexer->get($form_id)->get(0);

    $validator = $form_id . '_validate';
    if ($indexer->has($validator)) {
      $this->validator = $indexer->get($validator)->get(0);
    }
    $submit_handler = $form_id . '_submit';
    if ($indexer->has($submit_handler)) {
      $this->submitHandler = $indexer->get($submit_handler)->get(0);
    }
  ...
  public function render() {
    if (empty($this->controller)) {
      ...
      $this->controller = Parser::parseSource(render($render))
